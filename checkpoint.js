...
LinkedList.prototype.orderList = function () {
  ...
  const saveInCache = (head) => {
    if (head) {
      let headWhitoutNext = Object.assign({}, head);
      headWhitoutNext.next = null;
      cache.push(headWhitoutNext.value);
      saveInCache(head.next);
    }
  }
  saveInCache(this.head);
...
function joinLinkedList(linkedListOne, linkedListTwo) {
  ...
  const saveInCache = (head, cache) => {
    if (head) {
      let headWhitoutNext = Object.assign({}, head);
      headWhitoutNext.next = null;
      cache.push(headWhitoutNext.value);
      saveInCache(head.next, cache);
    }
    return cache;
  }
  saveInCache(linkedListOne.head, cacheOne);
  saveInCache(linkedListTwo.head, cacheTwo);

  let arrLong;

  if (cacheOne.length <= cacheTwo.length) arrLong = cacheTwo;
  else arrLong = cacheOne;
...
BinarySearchTree.prototype.searchMin = function () {
  let cache = [];
  let tree = { value: this.value, left: this.left, right: this.right };

  function breadthFirstForEach(tree, cache, arr = []) {
    cache.push(tree.value);

    if (tree.left) arr.push(tree.left);
    if (tree.right) arr.push(tree.right);
    if (arr.length) breadthFirstForEach(arr.shift(), cache, arr);
  }

  breadthFirstForEach(tree, cache);
...
function createBST(array) {
  let node = new BinarySearchTree(array[0]);
